...
**E2E test suite scaffolded (2026-05-05):** `tests/` at repo root with 6 browser-based test cases varied by clarity, size, and edge type (small+clear, mid+multi, large+render+HOA, sparse-but-valid, out-of-catalog, conflicting-info). Each case has expected outcomes, required scope items, MUST-NOT-includes, voice expectations, and a per-case pass threshold. Shared 100-point rubric in `tests/EVALUATION-RUBRIC.md` covers status/total/items/ambiguities/render/required-items/no-hallucination/voice/cost-time. Sub-agent prompt template in `tests/SUB-AGENT-INSTRUCTIONS.md` for spawning 3 runners (2 cases each). Results write to `tests/results/<case-id>-<runner>-<timestamp>.md`. ~$1-2 total API cost across all 6 runs. Suite scaffolded but not yet executed — runners deploy separately.
