...
**E2E test suite EXECUTED (2026-05-05):** 3 sub-agents ran 6 cases in parallel (15 min wall-clock, $0.90 total Anthropic spend). **6/6 PASS, average 89.2/100 — Excellent tier.** All math integrity / customer name / render flag / no-hallucination guards held across every case. Three cross-cutting findings documented in `tests/SUMMARY.md`: (1) address dedup bug — customer find-or-create silently overwrites form-submitted address with stored address (real product issue, recoverable); (2) ambiguity-pricing decoupling — agent can flag "should this be included?" AND price the item (case 02 bubbling rock); (3) conflict ambiguities sometimes surface in proposal prose only, not in structured panel (case 06). **System is Loom-ready.** Standout cases: 04 (sparse — perfect restraint, agent declined to fabricate dimensions), 05 (out-of-catalog — pizza oven flagged as BLOCKER, NOT priced, narrative committed to "revised number within 48 hours"). See `tests/SUMMARY.md` for full breakdown + Loom narrative recommendations.

**Post-test fix shipped (D42, 2026-05-05):** Case 03's validation_failed root cause addressed. Custom items (catalog-misses) are now strictly internal-only — never appear in customer-facing proposal. New "Needs your pricing" UI card on the quote review page surfaces them to Marcus alongside Ambiguities. Marcus decides per item: add manual line item or follow up separately. Re-ran case 03 against prod after deploy: status went `validation_failed` → `draft_ready` first try, 3 errors → 0 errors, total integrity preserved ($81,445), 4 custom items correctly surfaced via API. The agent's settled behavior: custom items get routed to Exclusions in the customer-facing proposal (managing expectations) AND to the Marcus-only UI card (action surface). Validation now passes with 2 voice warnings only (low priority).
