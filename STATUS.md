...
**Post-review feature (2026-05-05):** `POST /api/line-items` now accepts manual catalog additions. Newly inserted items are immediately available to the agent (no skill change — `match_pricing` queries the live DB on every run). **UPDATE: UI form shipped + dynamic categories shipped (D39)** — `/admin/line-items` has a combobox-based form that supports both adding items to existing categories AND creating brand-new categories. Migration 005 converted `line_items.category` from enum → text; `match_pricing.getCategories()` fetches live categories at run time so new categories are agent-searchable on the next quote with zero code change. Verified live with a new "outdoor_lighting" category.
