...
**You are operating on the user's SchilderGroei production server.** Before any action that could affect that:

1. **Confirm with user** that you have permission to operate on `157.90.124.14`
2. **Confirm port assignment policy** — what port range is OK to use? (Default: try 3100, 3101, etc. until free)
3. **Confirm public URL format** — IP:port (default for temporary 1-week deploy) OR subdomain (`quote-agent.tunderman.io` — adds ~30 min DNS work via Cloudflare)?

## STEP 2 — Critical constraints

**This server hosts SchilderGroei production.** Per `~/Desktop/system/credentials.md`:
- Active services: `schildergroei-api`, `schildergroei-web`, `willemschilderwerken-api`
- Existing convention: `/var/www/<domain>` for static, `/opt/<service>` for backends
- Caddy is the reverse proxy — already running with config for SchilderGroei domains
- This deploy is **TEMPORARY (~1 week)** per user

**Do NOT:**
- Touch anything in `/opt/schildergroei/` or any SchilderGroei-related path
- Modify any SchilderGroei systemd service (`schildergroei-*`)
...
- Test that existing SchilderGroei services are still running after each change

## STEP 3 — Tasks

Phase 0 of the deploy:

1. **SSH in:** `ssh -i ~/.ssh/id_ed25519 root@157.90.124.14`
2. **Snapshot state:**
...
3. **Verify Node 20+:**
   - `node --version` — if missing or <20, install via `nvm` (don't apt install — keeps SchilderGroei's Node version intact if different)
4. **Create app dir:**
   - `mkdir -p /opt/greenscape-quote-agent`
   - `chown <user>:<user> /opt/greenscape-quote-agent` (use the same user as SchilderGroei's services for consistency)
...
6. **Add Caddy site block:** append to `/etc/caddy/Caddyfile` (or create `/etc/caddy/conf.d/greenscape-quote-agent.conf` if site uses includes)
   ...
   - For subdomain: `quote-agent.tunderman.io { reverse_proxy localhost:<internal_port> }`
...
9. **Test with hello-world Next.js:**
   ...
   - Curl from outside: `curl http://157.90.124.14:<port>` should return Next.js page
...
- **Public URL** (e.g., `http://157.90.124.14:3100` or `https://quote-agent.tunderman.io`)
...
- [ ] `curl http://157.90.124.14:<port>` (or curl the subdomain) → 200 from outside
- [ ] `systemctl status schildergroei-api` → still active (didn't break SchilderGroei)
...
- **Hard rules from CLAUDE.md:**
  ...
  - Server 2 (`138.199.212.7`) is Lead System ONLY — do not deploy here
...
- [ ] Existing SchilderGroei services unaffected (verified)
